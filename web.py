...
def full_payload(ticker: str) -> dict:
    ...
    return {
        ...
        "tech": tech, "news": news,
        "checklist": [
            {"ok": score["total"] >= 5000, "label": "Скоринг > 5000 баллов",
             "actual": f"{score['total']}"},
            {"ok": fv.get("upside") is not None and fv["upside"] > 15,
             "label": "Апсайд > 15%",
             "actual": (f"{fv['upside']:.1f}%" if fv.get("upside") is not None else "N/A")},
            {"ok": data.get("short_pct") is not None and data["short_pct"]*100 < 5,
             "label": "Short Interest < 5%",
             "actual": (f"{data['short_pct']*100:.1f}%" if data.get("short_pct") is not None else "N/A")},
            {"ok": data.get("de") is not None and data["de"] < 3,
             "label": "D/E < 3x",
             "actual": (f"{data['de']:.2f}x" if data.get("de") is not None else "N/A")},
            {"ok": data.get("roe") is not None and data["roe"]*100 > 10,
             "label": "ROE > 10%",
             "actual": (f"{data['roe']*100:.1f}%" if data.get("roe") is not None else "N/A")},
            {"ok": (price is not None and hi is not None and (hi-price)/hi*100 > 10),
             "label": "Цена ниже 52-нед. максимума на 10%+",
             "actual": (f"{(hi-price)/hi*100:.0f}%" if (price and hi) else "N/A")},
        ],
    }
